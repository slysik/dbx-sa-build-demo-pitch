...
def main():
    try:
        ...
        if args.inject_context:
            ...
            # Also inject recent lessons from tasks/lessons.md
            lessons_path = Path(cwd) / "tasks" / "lessons.md"
            if lessons_path.exists():
                try:
                    lessons_content = lessons_path.read_text().strip()
                    if lessons_content:
                        # Slice last 1500 chars (most recent lessons)
                        if len(lessons_content) > 1500:
                            lessons_content = "...\n" + lessons_content[-1500:]
                        lessons_part = f"## Lessons Learned\n{lessons_content}"
                        context = f"{context}\n\n---\n\n{lessons_part}" if context else lessons_part
                except Exception:
                    pass
