...
def main() -> None:
    try:
        ...
        # Deregister agent from active tracker for status line display
        try:
            sys.path.insert(0, str(Path(__file__).parent))
            from utils.agent_tracker import deregister_agent
            agent_id_for_tracker = input_data.get("agent_id", "unknown")
            deregister_agent(agent_id_for_tracker)
        except Exception:
            pass  # Fail silently
