...
def main() -> None:
    try:
        ...
        # Register agent in active tracker for status line display
        try:
            from utils.agent_tracker import register_agent
            register_agent(agent_id, agent_type)
            debug_log(f"Registered agent in tracker: {agent_id} ({agent_type})")
        except Exception as e:
            debug_log(f"Failed to register agent in tracker: {e}")
