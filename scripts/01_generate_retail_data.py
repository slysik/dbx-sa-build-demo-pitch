"""
Step 1: Generate 1000 retail streaming JSON records using PySpark + Faker.
Saves as JSON to a Unity Catalog Volume for Zerobus trickle ingestion.
Demonstrates 3 Vs: Volume (1000 records), Variety (multiple types), Velocity (streaming trickle).
"""
import json
import numpy as np
...
CATALOG = "dbx_weg"
SCHEMA = "bronze"
VOLUME_PATH = f"/Volumes/{CATALOG}/{SCHEMA}/streaming_data"
TABLE_NAME = f"{CATALOG}.{SCHEMA}.retail_streaming_events"
N_RECORDS = 1000
SEED = 42
...
# Create volume for staging JSON data
spark.sql(f"CREATE VOLUME IF NOT EXISTS {CATALOG}.{SCHEMA}.streaming_data")

# =============================================================================
# GENERATE 1000 RETAIL RECORDS (PySpark + Faker)
# =============================================================================
# Product catalog with realistic categories and price ranges
PRODUCTS = {
    "Electronics": [
...
# Category weights (Zipf-like: Electronics most popular)
CATEGORIES = list(PRODUCTS.keys())
CATEGORY_WEIGHTS = [0.30, 0.25, 0.20, 0.15, 0.10]

# Generate records using pandas for Faker, then convert to Spark
records = []
base_time = datetime.now()

for i in range(N_RECORDS):
    # Deterministic randomness using hash-based approach
    cat_idx = abs(hash(f"{i}-cat-{SEED}")) % len(CATEGORIES)
    # Use weighted selection via numpy
    ...
    # Log-normal distribution for price within range
    raw_price = np.random.lognormal(mean=0, sigma=0.5)
    unit_price = round(min_price + (max_price - min_price) * min(raw_price / 3.0, 1.0), 2)

    # Poisson for quantity (lambda=2, most orders 1-3 items)
    quantity = max(1, int(np.random.poisson(lam=2)))
    total_amount = round(unit_price * quantity, 2)

    # Time spread: records span the last 2 hours with exponential inter-arrival
    seconds_ago = np.random.exponential(scale=7.0)  # avg 7 sec between events
...
print(f"Payment methods: {set(r['payment_method'] for r in records)}")
print(f"Price range: ${min(r['unit_price'] for r in records):.2f} - ${max(r['unit_price'] for r in records):.2f}")

# =============================================================================
# SAVE AS JSON TO VOLUME (for Zerobus trickle ingestion)
# =============================================================================
# Save as individual JSON lines file, partitioned into 100-record batches
BATCH_SIZE = 50  # 20 batches of 50 records = trickle every 10s
for batch_idx in range(0, N_RECORDS, BATCH_SIZE):
    ...
    dbutils.fs.put(
        f"{VOLUME_PATH}/batch_{batch_num:03d}.json",
        json_content,
        overwrite=True
    )
...
print(f"Ready for Zerobus trickle ingestion (1 batch every 10 seconds)")

# Also save full dataset for reference
full_json = "\n".join(json.dumps(r) for r in records)
dbutils.fs.put(f"{VOLUME_PATH}/all_records.json", full_json, overwrite=True)
print(f"Full dataset saved to {VOLUME_PATH}/all_records.json")
