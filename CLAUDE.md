...
- **NEVER hardcode secrets** — no fallback values in `os.environ.get("KEY", "hardcoded")`. Store all keys in `~/.zshrc`. Use `git-filter-repo` to purge if leaked. GitHub detects `dose*` (Databricks OAuth) and `sk-ant-api03-*` (Anthropic) within 3 min of push. **Secret rotation incident 2026-03-25:** backup SP secret leaked in CLAUDE.md curl example → GitHub flagged within minutes → purged with `git-filter-repo --replace-text`, force pushed, rotated secret in Databricks UI, updated `~/.databrickscfg` + `~/.zshrc`. Dismiss alerts at `github.com/{repo}/security/secret-scanning`.
- **Secret rotation workflow** — (1) remove from file, (2) commit, (3) `git-filter-repo --replace-text <file-with-secret> --force`, (4) re-add remote (`git remote add origin ...`), (5) `git push --force`, (6) rotate in Databricks UI → Service Principals → OAuth secrets, (7) update `~/.databrickscfg` + `~/.zshrc`, (8) verify with curl token request, (9) dismiss GitHub alert at `/security/secret-scanning`.
- **`git-filter-repo` removes remotes** — after running, must `git remote add origin <url>` again. Also creates `.git/filter-repo/already_ran` marker; delete it or pipe `echo Y` for subsequent runs.
- **Env var for backup SP secret** — `$BACKUP_SP_SECRET` in `~/.zshrc`. CLAUDE.md curl examples reference this var, never the literal value.
